...
def generate_episode(
    ...
    state_shape = tuple(int(size) for size in torch.as_tensor(states).shape[1:])
...
def apply_boundary_read(
    ...
    shape = tuple(int(size) for size in state_t.shape)
    space = loci.coordinate_space(shape, steps=1)
    boundary = {**dict(boundary), "coordinate_space": space}
    coord_t = torch.tensor(tuple(int(value) for value in coord), dtype=torch.long)
    return loci.gather(coord_t.reshape(1, 4), state_t.unsqueeze(0), boundary)[0]
...
def apply_rule(rule: Any, reads: Sequence[Any], rule_id: int) -> Any:
    ...
    if instantiated.family in {"dyadrads_1d", "dyadaxes_2d", "dyadaxes_3d"}:
        ...
        for channel in instantiated.channels:
            component_reads = torch.as_tensor(reads[channel.component])
...
def _rollout_ar2(initial_state: Any, rule: rules.Rule, steps: int) -> torch.Tensor:
    ...
    seed = torch.as_tensor(initial_state, dtype=torch.long).reshape(-1)
    if seed.numel() != 2:
        raise ValueError(f"AR2 initial_state must contain 2 values, got {seed.numel()}")

    previous = int(seed[0])
    current = int(seed[1])
    states = torch.empty((int(steps),), dtype=torch.long)
    states[0] = current
...
def _rollout_spatial_lookup(
    ...
    state = torch.as_tensor(initial_state, dtype=torch.long)
    if state.ndim < 1 or state.ndim > 3:
        raise ValueError(f"spatial state rank must be 1..3, got {state.ndim}")

    states = torch.empty((int(steps), *state.shape), dtype=torch.long, device=state.device)
    states[0] = state
...
def _next_spatial_state(
    ...
    index = _lookup_index(channel_bits)
    rule_id = torch.as_tensor(int(rule.rule_id), dtype=torch.long, device=index.device)
    return torch.bitwise_and(torch.bitwise_right_shift(rule_id, index), 1).reshape(state.shape)
...
def _read_component(
    ...
    space = loci.coordinate_space(shape, steps=1)
    current_coords = loci.absolute_universe(space, t=0).to(state.device)
    ...
    offsets = offsets.to(state.device)
...
def _channel_state(reads: torch.Tensor, channel: rules.RuleChannel) -> torch.Tensor:
    values = torch.as_tensor(reads)
    ...
    for step in channel.pipeline:
        ...
        if rule_type == "exhaustive":
            if values.shape[-1] != 1:
                ...
                weights = alphabet_size ** torch.arange(
                    values.shape[-1],
                    dtype=torch.long,
                    device=values.device,
                )
                state = (values.long() * weights).sum(dim=-1)
            else:
                state = values[..., 0].long()
            continue

        if rule_type == "totalistic":
            state = values.long().sum(dim=-1)
            continue
...
def _apply_gate(values: torch.Tensor, params: Mapping[str, Any], group_size: int) -> torch.Tensor:
    ...
    if gate_type == "ceil":
        return torch.ceil(values.float()).long()
    if gate_type == "floor":
        return torch.floor(values.float()).long()

    raise NotImplementedError(f"unsupported gate type {gate_type!r}")


def _lookup_index(channel_bits: Sequence[Any]) -> torch.Tensor:
    index = None

    for bit_index, bit_values in enumerate(channel_bits):
        bit_tensor = torch.as_tensor(bit_values, dtype=torch.long)
